...
export const createProjectPruductsWithCsv = async (excelId: Types.ObjectId , project: IProject , assignerRole : string) => {
  const thisFile = await fileRepo.findOne({
    _id: excelId,
    type: "text/csv",
  });
  console.log("thisFile", assignerRole);
  if (!thisFile) {
    return {
  ...
  console.log("محتوای اکسل:", jsonData);
